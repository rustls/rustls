...
fn unoffered_alpn_test(check_selected_alpn: bool) -> Result<rustls::IoState, Error> {
    ...
    let state = client
        .read_tls(&mut input, &mut client_output)
...
fn server_does_not_negotiate_tls12_after_hrr() {
    ...
    let mut server_output = Vec::new();
    let mut input = encoding::record_framing(
        ContentType::Handshake,
        ProtocolVersion::TLSv1_2,
...
fn test_tls13_valid_early_plaintext_alert() {
    ...
    assert_eq!(
        server
            .read_tls(&mut SliceInput::new(&mut alert), &mut server_output)
            .handle_all(&mut Vec::new())
            .unwrap_err(),
...
fn test_client_ech_without_hpke_suites() {
    ...
    assert_eq!(
        ...
        concat!(
            "ApiMisuse error not found while calling with_ech method on config ",
            "where with_ech_hpke_suites hadn't been called",
        )
    );
    ...
    assert_eq!(
        ...
        Error::ApiMisuse(ApiMisuse::NoEchHpkeSuites),
        concat!(
            "ApiMisuse error not found while calling with_ech_grease method on config ",
            "where with_ech_hpke_suites hadn't been called",
        )
    );
    ...
    /*assert_eq!(
        config
            .connect("example.com".try_into().unwrap())
            .with_ech_for_retry(RejectedEch {
                retry_configs: None
            })
            .err(),
        Some(Error::ApiMisuse(ApiMisuse::NoEchHpkeSuites)),
        "ApiMisuse error not found while calling with_ech_grease method on {config_msg}",
    );*/
}

#[test]
fn test_client_ech_with_empty_hpke_suites_slice() {
    assert_eq!(
        ClientConfig::builder(provider::DEFAULT_TLS13_PROVIDER.into())
            .with_ech_hpke_suites(&[])
            .err(),
        Some(Error::ApiMisuse(ApiMisuse::EmptyEchHpkeSuitesListFound))
    );
}
...
fn tls13_packed_handshake() {
    ...
    assert_eq!(
        client
            .read_tls(&mut SliceInput::new(&mut second_flight), &mut client_output)
            .handle_all(&mut Vec::new())
            .unwrap_err(),
