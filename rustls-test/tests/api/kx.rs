...
use rustls::{ClientConfig, Connection, HandshakeKind, ServerConfig, VecBuffer};
use rustls_test::{
...
fn hybrid_kx_component_share_offered_but_server_chooses_something_else() {
    ...
    let mut client_1_buf = VecBuffer::default();
    let mut server_buf = VecBuffer::default();

    // client_2 supplies the ClientHello, client_1 receives the ServerHello
    transfer(&mut client_2, &mut server_buf);
    server
        .process_new_packets(&mut server_buf)
        .unwrap();
    transfer(&mut server, &mut client_1_buf);
    assert_eq!(
        client_1
