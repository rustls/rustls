...
fn server_external_verifier_does_not_call_trait() {
        ...
        }))
    {
        ...
        let identity = verify.asserted_identity().unwrap();
        assert_eq!(
            identity.identity,
            expect
                .key_type
        ...
        let ServerHandshake::NeedsInput(server) = verify
            .into_continue(Ok(verified), &mut server_output)
            .unwrap()
        else {
...
fn server_external_verifier_error_sends_alert() {
        ...
        MultiTest::new(provider::DEFAULT_PROVIDER).require_client_auth()
    {
        ...
        let err = verify
            .into_continue(Err(CertificateError::Expired.into()), &mut alert_output)
            .unwrap_err();
