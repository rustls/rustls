...
fn buffered_client_data_sent() {
    ...
    for version_provider in ALL_VERSIONS {
        ...
        let mut client_buf = VecInput::default();
        let mut server_buf = VecInput::default();
...
fn test_client_write_and_vectored_write_equivalence() {
    ...
    let bytes_written_chunked = client
        ...
        .unwrap();
    let bytes_sent_chunked = transfer(&mut client, &mut server_buf);
    ...
    let bytes_written_contiguous = client
        ...
        .unwrap();
    let bytes_sent_contiguous = transfer(&mut client, &mut server_buf);
...
fn test_write_vectored_degenerate_cases() {
    ...
    assert_eq!(transfer(&mut client, &mut server_buf), 0);
    assert_eq!(transfer(&mut server, &mut client_buf), 0);
    ...
    assert!(transfer(&mut client, &mut server_buf) > 0);
    assert!(transfer(&mut server, &mut client_buf) > 0);
    server.process_new_packets(&mut server_buf).unwrap();
    client.process_new_packets(&mut client_buf).unwrap();
...
fn test_acceptor() {
    ...
    let mut acceptor_buf = VecInput::default();
    acceptor_buf
...
fn test_plaintext_buffer_limit(limit: Option<usize>, plaintext_limit: usize) {
    ...
    let mut client_tls_buf = VecInput::default();
    let mut server_tls_buf = VecInput::default();
