...
fn client_handshake_receives_half_rtt_data() {
    ...
    server
        .write_tls(b"first half-rtt message".into(), &mut server_output)
        .unwrap();
    server
        .write_tls(b"second half-rtt message".into(), &mut server_output)
        .unwrap();
    ...
    let state = split
        .receive
        .read(&mut input, &mut client_output)
        .unwrap();
...
fn test_acceptor_continues_tls13_hrr_with_compatibility_ccs() {
    ...
    client
        .process_new_packets(&mut SliceInput::new(&mut output), &mut client_output)
        .handle_all(&mut Vec::new())
        .unwrap();
    output.clear();
