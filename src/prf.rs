...
fn p(hashalg: &'static digest::Algorithm,
     secret: &[u8],
     seed: &[u8],
     len: usize) -> Vec<u8> {
  ...
  while ret.len() < len {
    ...
    ret.extend_from_slice(p_term.as_ref());
...
pub fn prf(hashalg: &'static digest::Algorithm,
           secret: &[u8],
           label: &[u8],
           seed: &[u8],
           len: usize) -> Vec<u8> {
  let joined_seed = concat(label, seed);
  p(hashalg, secret, &joined_seed, len)
}
...
mod tests {
  ...
  fn check_sha256() {
    ...
    let output = super::prf(&SHA256, secret, label, seed, expect.len());
    assert_eq!(expect.len(), output.len());
    assert_eq!(expect.to_vec(), output);
  }
  ...
  fn check_sha512() {
    ...
    let output = super::prf(&SHA512, secret, label, seed, expect.len());
    assert_eq!(expect.len(), output.len());
    assert_eq!(expect.to_vec(), output);
  }
}
