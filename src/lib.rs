...
impl UnknownExtension {
  ...
  fn decode(typ: ExtensionType, r: &mut Reader) -> UnknownExtension {
    UnknownExtension {
      typ: typ,
      payload: UnknownPayload::decode(r)
    }
  }
}
...
impl Codec for ECPointFormatList {
  ...
  fn decode(r: &mut Reader) -> ECPointFormatList {
    codec::decode_vec_u8::<ECPointFormat>(r)
  }
}
...
impl Codec for EllipticCurveList {
  ...
  fn decode(r: &mut Reader) -> EllipticCurveList {
    codec::decode_vec_u16::<NamedCurve>(r)
  }
}
...
impl Codec for SignatureAndHashAlgorithm {
  ...
  fn decode(r: &mut Reader) -> SignatureAndHashAlgorithm {
    SignatureAndHashAlgorithm {
      hash: HashAlgorithm::decode(r),
      signature: SignatureAlgorithm::decode(r)
    }
  }
}
...
impl Codec for SupportedSignatureAlgorithms {
  ...
  fn decode(r: &mut Reader) -> SupportedSignatureAlgorithms {
    codec::decode_vec_u16::<SignatureAndHashAlgorithm>(r)
  }
}
...
impl Codec for ClientExtension {
  ...
  fn decode(r: &mut Reader) -> ClientExtension {
    ...
    match typ {
      ExtensionType::ECPointFormats => ClientExtension::ECPointFormats(ECPointFormatList::decode(&mut sub)),
      ExtensionType::EllipticCurves => ClientExtension::EllipticCurves(EllipticCurveList::decode(&mut sub)),
      ExtensionType::SignatureAlgorithms => ClientExtension::SignatureAlgorithms(SupportedSignatureAlgorithms::decode(&mut sub)),
      _ => ClientExtension::Unknown(UnknownExtension::decode(typ, &mut sub))
    }
  }
}
...
impl Codec for ServerExtension {
  ...
  fn decode(r: &mut Reader) -> ServerExtension {
    let typ = ExtensionType::decode(r);
    let len = codec::decode_u16(r) as usize;
    let mut sub = r.sub(len);

    match typ {
      ExtensionType::ECPointFormats => ServerExtension::ECPointFormats(ECPointFormatList::decode(&mut sub)),
      _ => ServerExtension::Unknown(UnknownExtension::decode(typ, &mut sub))
    }
  }
}
...
impl Codec for ServerHelloPayload {
  ...
  fn decode(r: &mut Reader) -> ServerHelloPayload {
    let mut ret = ServerHelloPayload {
      server_version: ProtocolVersion::decode(r),
      random: Random::decode(r),
      session_id: SessionID::decode(r),
      cipher_suite: CipherSuite::decode(r),
      compression_method: Compression::decode(r),
      extensions: Vec::new()
    };

    if r.any_left() {
      ret.extensions = codec::decode_vec_u16::<ServerExtension>(r);
    }
...
impl Codec for HandshakeMessagePayload {
  ...
  fn decode(r: &mut Reader) -> HandshakeMessagePayload {
    ...
    let payload = match typ {
      ...
      HandshakeType::ServerHello => HandshakePayload::ServerHello(ServerHelloPayload::decode(&mut sub)),
