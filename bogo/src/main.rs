...
use rustls::{Connection, DistinguishedName, HandshakeKind, RootCertStore, VecBuffer, compress};
...
fn read_n_bytes(
    ...
    match conn.read(&mut bytes[..n]) {
        Ok(count) => {
            ...
            buf.read(&mut io::Cursor::new(&mut bytes[..count]))
...
fn after_read(
    ...
    //dbg!("process new packets", buf.slice_mut().len());
