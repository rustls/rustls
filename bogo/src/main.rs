...
impl Credentials {
    fn most_recent_mut(&mut self) -> &mut Credential {
        self.additional
            .last_mut()
...
pub fn main() {
    ...
    while !args.is_empty() {
        ...
        match arg.as_ref() {
            ...
            "-key-file" => {
                opts.credentials.most_recent_mut().key_file = args.remove(0);
            }
            "-new-x509-credential" => {
            ...
            "-cert-file" => {
                opts.credentials.most_recent_mut().cert_file = args.remove(0);
            }
            "-trust-cert" => {
            ...
            "-signing-prefs" => {
                ...
                opts.credentials.most_recent_mut().use_signing_scheme = Some(alg);
            }
            "-must-match-issuer" => {
                opts.credentials.most_recent_mut().must_match_issuer = true;
            }
            "-use-client-ca-list" => {
