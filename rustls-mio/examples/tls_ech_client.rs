...
use rustls::{ClientConnection, ConfigBuilder, RootCertStore, ServerIdentity, Connection};
...
fn main() {
    ...
    let ech = EncryptedClientHello::with_host_and_config_list(dns_name, &config.to_bytes().unwrap()).unwrap();
    ...
    let mut connection = ClientConnection::with_server_id(Arc::new(client_config), ServerIdentity::EncryptedClientHello(ech)).unwrap();
