...
mod client {
    ...
    use rustls::{ClientConfig, Error, TlsInputBuffer};
...
mod server {
    ...
    use rustls::{Connection, DistinguishedName, ServerConfig, ServerConnection, TlsInputBuffer};
    use rustls_aws_lc_rs as provider;
        ...
        loop {
            match conn.reader().read(&mut buf) {
                ...
                Err(err) if err.kind() == ErrorKind::WouldBlock => {
                    input.read(&mut stream, conn.is_handshaking())?;
