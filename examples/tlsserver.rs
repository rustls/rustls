...
`echo' mode means the server echoes \
     received data on each connection.

`http' mode means the server blindly sends a HTTP \
     response on each connection.

`forward' means the server forwards plaintext to a connection \
     made to
localhost:fport.
...
Usage:
  ...
  tlsserver --certs CERTFILE --key KEYFILE [--suite \
     SUITE ...] [--proto PROTO ...] [options] forward <fport>
  tlsserver (--version | -v)
  \
     tlsserver (--help | -h)
...
Options:
    ...
    \
     --certs CERTFILE    Read server certificates from CERTFILE.
                        This \
     should contain PEM-format certificates
                        in the right order (the first \
     certificate should
    ...
    --proto PROTOCOL    Negotiate PROTOCOL using \
     ALPN.
