...
fn load_rpk(path_to_rpk: &str) -> Vec<SubjectPublicKeyInfoDer<'static>> {
...
fn main() -> Result<(), io::Error> {
    ...
    let server_spki_certs = load_rpk(&server_spki_file)
        .into_iter()
        .last()
        .unwrap();
    let server_private_key = load_private_key(&server_private_key_file).unwrap();
    let client_spki_certs = load_rpk(&client_spki_file)
        .into_iter()
        .last()
        .unwrap();
