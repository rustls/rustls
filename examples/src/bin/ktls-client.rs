...
mod linux {
    ...
    use rustls::kernel::KernelClientConnection;
    ...
    fn connect(
        ...
        sock: &mut TcpStream,
    ) -> Result<KernelClientConnection, Box<dyn Error>> {
        ...
        let (secrets, external) = conn.dangerous_into_kernel_connection()?;
    ...
    fn handle_control_message(
        sock: &mut TcpStream,
        conn: &mut KernelClientConnection,
        data: &mut Vec<u8>,
        ...
        match ContentType::from(cmsg.data()[0]) {
            ...
            // - new session tickets
            //
            // We need to use the methods on KernelConnection to handle both.
