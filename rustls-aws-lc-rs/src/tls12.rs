            ...
            |nonce, aad, payload, ciphertext_position| {
                self.dec_key
                    .open_in_place(
                        ...
                        aead::Aad::from(aad),
                        &mut payload[ciphertext_position..],
                    )
                    .map(|plaintext| ciphertext_position..ciphertext_position + plaintext.len())
...
impl RecordEncrypter for GcmRecordEncrypter {
    fn encrypt<'a>(
        ...
        let enc_key = RefCell::new(&mut self.enc_key);
        ...
        gcm_encrypt_record(
            ...
            Some(
                |nonce: Nonce, aad, plaintext: &[u8], ciphertext: &mut [u8], tag: &mut [u8]| {
                    enc_key
                        .borrow_mut()
...
mod tests {
    ...
    use rustls::crypto::cipher::{
        EncodableVersion, GCM_EXPLICIT_NONCE_LEN, InboundOpaque, NONCE_LEN,
    };
