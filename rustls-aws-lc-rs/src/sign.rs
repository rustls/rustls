...
impl RsaPssSigningKey {
    ...
    pub(super) fn from_pkcs8(pkcs8: &[u8]) -> Option<Self> {
        ...
        // aws-lc-rs's RsaKeyPair::from_pkcs8 only accepts the rsaEncryption
        // OID, so we extract the raw RSAPrivateKey from the PKCS#8 OCTET
        // STRING and use from_der instead.
