...
fn pkcs8_pss_hash(pkcs8: &[u8]) -> Option<PssHash> {
    ...
    // We confirmed the OID. Now match the full AlgorithmIdentifier
    // (the SEQUENCE we entered, not just the OID) against known constants
    // by reconstructing its DER encoding.
    //
    // The `rest` pointer is right at the AlgorithmIdentifier SEQUENCE.
    // We need its complete TLV (tag + length + value). Extract it.
