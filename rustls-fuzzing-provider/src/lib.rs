...
use rustls::{
    crypto, server, sign, ConnectionTrafficSecrets, ContentType, Error, PeerMisbehaved,
    ProtocolVersion, RootCertStore, SignatureAlgorithm, SignatureScheme,
};
use webpki::alg_id;
...
static ALL_CIPHER_SUITES: &[rustls::SupportedCipherSuite] =
    &[TLS13_FUZZING_SUITE, TLS_FUZZING_SUITE];

static TLS13_FUZZING_SUITE: rustls::SupportedCipherSuite =
    rustls::SupportedCipherSuite::Tls13(&rustls::Tls13CipherSuite {
        common: rustls::crypto::CipherSuiteCommon {
            suite: rustls::CipherSuite::Unknown(0xff13),
            hash_provider: &Hash,
            confidentiality_limit: u64::MAX,
        },
        hkdf_provider: &rustls::crypto::tls13::HkdfUsingHmac(&Hmac),
        aead_alg: &Aead,
        quic: None,
    });

static TLS_FUZZING_SUITE: rustls::SupportedCipherSuite =
    rustls::SupportedCipherSuite::Tls12(&rustls::Tls12CipherSuite {
        common: rustls::crypto::CipherSuiteCommon {
            suite: rustls::CipherSuite::Unknown(0xff12),
            hash_provider: &Hash,
            confidentiality_limit: u64::MAX,
        },
        kx: rustls::crypto::KeyExchangeAlgorithm::ECDHE,
        sign: &[SIGNATURE_SCHEME],
        prf_provider: &rustls::crypto::tls12::PrfUsingHmac(&Hmac),
        aead_alg: &Aead,
    });
...
impl crypto::ActiveKeyExchange for ActiveKeyExchange {
    fn complete(self: Box<Self>, peer: &[u8]) -> Result<crypto::SharedSecret, Error> {
        if peer == KX_PEER_SHARE {
            Ok(crypto::SharedSecret::from(KX_SHARED_SECRET))
        } else {
            Err(Error::from(PeerMisbehaved::InvalidKeyShare))
        }
    }
    ...
    fn group(&self) -> rustls::NamedGroup {
        rustls::NamedGroup::from(0xfe00)
    }
}

const ALL_KX_GROUPS: &[&dyn crypto::SupportedKxGroup] =
    &[&KeyExchangeGroup as &dyn crypto::SupportedKxGroup];
...
impl sign::SigningKey for SigningKey {
    fn choose_scheme(&self, offered: &[SignatureScheme]) -> Option<Box<dyn sign::Signer>> {
        if offered.contains(&SIGNATURE_SCHEME) {
            Some(Box::new(SigningKey))
        } else {
            None
        }
    }
