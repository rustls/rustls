...
use rustls::{ClientConfig, Connection, ServerConfig, ServerConnection, TlsInputBuffer};
...
fn test_version(provider: CryptoProvider) -> Transcript {
    ...
    let (mut client_buf, mut server_buf) = (TlsInputBuffer::default(), TlsInputBuffer::default());
    ...
    while client.is_handshaking() || server.is_handshaking() {
        ...
        server_buf
            .read(&mut &buffer[..], server.is_handshaking())
            .unwrap();
        server
        ...
        transcript
            ...
            .extend_from_slice(&buffer);
        client_buf
            .read(&mut &buffer[..], client.is_handshaking())
            .unwrap();
        client
