...
impl Extractor {
    pub(crate) fn without_salt(hmac: &'static dyn hmac::Hmac) -> Self {
        let zeroes = [0u8; hash::Output::MAX_LEN];
        Self::new(hmac, &zeroes[..hmac.hash_output_len()])
    }

    pub(crate) fn new(hmac: &'static dyn hmac::Hmac, salt: &[u8]) -> Self {
        Self {
    ...
    /// This is the `HKDF-Extract` step: takes the input keying material `ikm` and "extracts"
    /// from it a fixed-length pseudorandom key.
    ///
    /// The returned [`Expander`] can be used for the `HKDF-Expand` step, see:
    /// [`Expander::expand()`] et al.
...
/// This is a single "output keying material" (OKM) block output from HKDF-Expand.
#[derive(Clone)]
pub(crate) struct OkmBlock(hmac::Tag);

/// TODO: only required for quic tests
impl From<&[u8]> for OkmBlock {
...
impl Expander {
    pub(crate) fn from_okm(okm: &OkmBlock, hmac: &'static dyn hmac::Hmac) -> Self {
        Self(hmac.with_key(okm.0.as_ref()))
    }

    /// HKDF-Expand, writing into a slice.
    ///
    /// This returns an error if the slice is longer than the maximum HKDF-Expand
    /// L parameter.
    ...
    /// HKDF-Expand, producing one block of output.
    pub(crate) fn expand_block(&self, info: &[&[u8]]) -> OkmBlock {
        ...
        OkmBlock(hmac::Tag::new(reduced_tag))
    }

    /// This is the `HKDF-Expand` step: "expands" the key into several additional pseudorandom keys
    /// (the output of the KDF).
    ///
    /// Produces a type `T` which can be constructed from a byte array.
    ///
    /// This does not fail, on the assumption that the size of the byte array
    /// is less than 255 times the HMAC tag length.
