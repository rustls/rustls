...
use crate::msgs::{
    ...
    DTLS_HANDSHAKE_HEADER_SIZE, EpochAndSequence, HANDSHAKE_HEADER_SIZE, HandshakeMessagePayload,
...
fn single_handshake_fragment(version: ProtocolVersion, encrypted: bool) {
    ...
    .to_unencrypted_bytes(EncodingContext {
        ...
        epoch_and_sequence: Some(EpochAndSequence::new(0, 6)),
    });
    ...
    assert_eq!(epoch_and_sequence, Some(EpochAndSequence::new(0, 6)));
...
fn multiple_fragments_application_data(version: ProtocolVersion) {
    ...
    for (encoded_record, expect_start, expect_end, expect_epoch_and_sequence) in [
        (
