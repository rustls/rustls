...
fn refuses_client_exts_with_unparsed_bytes() {
    ...
    let mut rd = Reader::init(&bytes);
    assert!(ClientExtensions::read(&mut rd).is_err());
}
...
fn refuses_server_ext_with_unparsed_bytes() {
    let bytes = [0x00u8, 0x0b, 0x00, 0x04, 0x02, 0xf8, 0x01, 0x02];
    let mut rd = Reader::init(&bytes);
    assert!(ServerExtensions::read(&mut rd).is_err());
}

#[test]
fn refuses_certificate_ext_with_unparsed_bytes() {
    let bytes = [0x00u8, 0x07, 0x00, 0x05, 0x00, 0x03, 0x00, 0x00, 0x01];
    let mut rd = Reader::init(&bytes);
    assert!(CertificateExtensions::read(&mut rd).is_err());
}
...
fn refuses_certificate_req_ext_with_unparsed_bytes() {
    ...
    let mut rd = Reader::init(&bytes);
    assert!(CertificateRequestExtensions::read(&mut rd).is_err());
}
...
fn clientextensions_basics() {
    ...
    target.clone_one_from(&src, ExtensionType::EarlyData);
    assert_eq!(
