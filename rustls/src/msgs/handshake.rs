...
pub enum ClientExtension {
    ...
    ServerCertType(Vec<CertificateType>),
    ClientCertType(Vec<CertificateType>),
...
impl Codec<'_> for ClientExtension {
    fn encode(&self, bytes: &mut Vec<u8>) {
        ...
        match *self {
            ...
            Self::ClientCertType(ref r) => r.encode(nested.buf),
            Self::ServerCertType(ref r) => r.encode(nested.buf),
    ...
    fn read(r: &mut Reader<'_>) -> Result<Self, InvalidMessage> {
        ...
        let ext = match typ {
            ...
            ExtensionType::ClientCertificateType => Self::ClientCertType(Vec::read(&mut sub)?),
            ExtensionType::ServerCertificateType => Self::ServerCertType(Vec::read(&mut sub)?),
...
impl ClientHelloPayload {
    ...
    pub(crate) fn server_certificate_extension(&self) -> Option<&[CertificateType]> {
        ...
        match *ext {
            ClientExtension::ServerCertType(ref req) => Some(req),
            _ => None,
        }
    }
    ...
    pub(crate) fn client_certificate_extension(&self) -> Option<&[CertificateType]> {
        ...
        match *ext {
            ClientExtension::ClientCertType(ref req) => Some(req),
            _ => None,
        }
    }
...
pub(crate) trait HasServerExtensions {
    ...
    fn server_certificate_type(&self) -> Option<&CertificateType> {
        let ext = self.find_extension(ExtensionType::ServerCertificateType)?;
        match *ext {
    ...
    fn client_certificate_type(&self) -> Option<&CertificateType> {
        let ext = self.find_extension(ExtensionType::ClientCertificateType)?;
        match *ext {
