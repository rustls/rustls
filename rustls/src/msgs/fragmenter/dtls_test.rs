...
use crate::crypto::cipher::{EncodableVersion, EncodedMessage, Payload};
...
fn handshake_fragments_flush_with_record(version: ProtocolVersion) {
        ...
        .enumerate()
    {
        ...
        assert_eq!(message_seq, 11.into(), "fragment {idx}");
        assert_eq!(
            fragment_offset,
...
fn flight_handshake_fragments_flush_with_record(version: ProtocolVersion) {
    ...
    let messages = [(vec![6u8; 32], 17), (vec![7; 32], 18), (vec![8; 16], 19)];
    ...
    for (idx, (record, (message, seq))) in records.iter().zip(messages).enumerate() {
...
fn flight_handshake_fragments_span_record(version: ProtocolVersion) {
    ...
    //                  size
    let messages = [(vec![6u8; 36], 17), (vec![7; 32], 18)];
    let message_flight: Vec<_> = messages
        .iter()
        .map(|(m, seq)| {
            (
...
fn flight_partially_filled_record(version: ProtocolVersion) {
    ...
    let messages = [(vec![6u8; 28], 17u16), (vec![7; 4], 18), (vec![8; 16], 19)];
    let record_lens = [
