...
use crate::msgs::message::OpaqueMessageRecv;
...
fn test_read_fuzz_corpus() {
    ...
    for file in fs::read_dir(corpus_dir()).unwrap() {
        ...
        let (msg, rest) = OpaqueMessageRecv::read(&mut bytes).unwrap();
        let used = len - rest.len();
...
fn can_read_safari_client_hello() {
    ...
    let mut bytes = b"\
        ...
        .to_vec();
    let (m, _) = OpaqueMessageRecv::read(&mut bytes).unwrap();
    println!("m = {:?}", m);
