...
macro_rules! extension_struct {
    (
        ...
        impl<'a> $struct_name<'a> {
            ...
            /// Return `true` if this structure has any extensions.
            fn any_extensions(&self) -> bool {
                $(
                    if let Some(_) = &self.$item_slot {
                        return true;
                    }
            ...
            /// Copy the value of the extension identified by `typ` from `self` to `target`.
            ///
            ...
            pub(crate) fn copy_to(
                ...
                match typ {
                    $(
                        $item_id => target.$item_slot = self.$item_slot.clone(),
                    )*
                    _ => {},
                }
            }
