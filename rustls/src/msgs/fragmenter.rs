...
use crate::msgs::{
    ...
    EpochAndSequence, HEADER_SIZE, HandshakeSequenceNumber, U24,
};
...
const MAX_STREAM_FRAGMENT_LEN: usize = MAX_FRAGMENT_LEN + HEADER_SIZE;
...
const MAX_UDP_FRAGMENT_LEN: usize = MAX_FRAGMENT_LEN + DTLS_12_HEADER_SIZE;
...
impl MessageFragmenter {
    ...
    pub(crate) fn fragment_dtls_handshake_message<'a>(
        ...
        Chunker::new(
            ...
            self.max_frag - DTLS_HANDSHAKE_HEADER_SIZE,
        )
        .map(move |payload| {
    ...
    pub(crate) fn fragment_dtls_handshake_message_flight<'a>(
        ...
        let record_capacity = self.max_frag;
    ...
    ///
    /// This includes overhead for the record header. A `max_fragment_size` of 10 will produce TLS
    /// fragments up to 10 bytes long.
    ///
    ...
    pub(crate) fn set_max_fragment_size(
        ...
        self.max_frag = match (protocol, max_fragment_size) {
            (Protocol::Tcp | Protocol::Quic(_), Some(sz @ 32..=MAX_STREAM_FRAGMENT_LEN)) => {
                sz - HEADER_SIZE
            }
            (Protocol::Udp, Some(sz @ 32..=MAX_UDP_FRAGMENT_LEN)) => sz - DTLS_12_HEADER_SIZE,
