...
use crate::conn::{
    ...
    NeedsInput, ServerNext, SideData, TcpTransport, TlsInputBuffer, VerifyPeerIdentity,
};
#[cfg(doc)]
use crate::crypto;
...
impl TryFrom<Core<ServerSide, TcpTransport>> for ServerHandshake {
    type Error = Error;

    fn try_from(core: Core<ServerSide, TcpTransport>) -> Result<Self, Error> {
...
/// The handshake can be progressed by choosing a [`ServerConfig`] based on
/// [`Accepted::client_hello()`] and providing it to [`Accepted::choose_config()`].
pub struct Accepted(AcceptedCore<TcpTransport>);
