...
use crate::{
    ...
    msgs::{
        ...
        handshake::{HandshakeMessagePayload, HandshakePayload, NewSessionTicketPayload},
...
use crate::server::{
    common::{ClientCertDetails, HandshakeDetails, ServerKXDetails},
    hs,
};
...
impl hs::State for ExpectCertificate {
    ...
    fn handle(
        ...
        if cert_chain.is_empty() && !sess.config.verifier.client_auth_mandatory() {
...
impl hs::State for ExpectClientKX {
    ...
    fn handle(
        ...
        let kxd = kx.server_complete(&client_kx.0).ok_or_else(|| {
