...
use crate::common_state::{DropAppData, JoinOutput, Protocol, SendPath};
...
fn next_state(mut inner: Box<ConnectionCore<ServerSide>>) -> ServerState {
    ...
    if !inner
        ...
        .is_empty()
    {
        return ServerState::SendServerFlight(SendServerFlight { inner });
    }
    ...
    match &inner.state {
        ...
        ) => ServerState::Traffic(ServerTraffic::new(inner)),
        ...
        Ok(hs::ServerState::ChooseConfig(_)) => {
            ...
            ServerState::ChooseConfig(ChooseConfig {
                inner,
                choose_config,
            })
        }
