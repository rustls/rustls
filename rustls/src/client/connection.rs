...
use crate::client::{ClientTraffic, EchStatus, SendEarlyData};
...
impl ClientConnection {
    fn new(state: ClientState, fips: FipsStatus) -> Self {
    ...
    pub fn early_data(&mut self) -> Option<WriteEarlyData<'_>> {
        match &mut self.state {
            Ok(InnerState::Handshake(ClientState::AwaitServerFlight(st))) => {
    ...
    pub fn is_early_data_accepted(&self) -> bool {
        match &self.state {
            Ok(InnerState::Traffic { outputs, .. }) => outputs.is_early_data_accepted(),
            _ => false,
        }
    }
    ...
    fn post_handshake_state(&mut self, new: ClientState) -> InnerState {
        if let ClientState::Traffic(traffic) = new {
            let ClientTraffic {
                mut send,
                receive,
                outputs,
...
impl Connection for ClientConnection {
    ...
    fn process_new_packets(&mut self) -> Result<IoState, Error> {
        loop {
            ...
            match state {
                InnerState::Handshake(ClientState::SendClientFlight(mut scf)) => {
                    while let Some(chunk) = scf.take_data() {
                        self.buffers.sendable_tls.append(chunk);
                    }
                    self.state = Ok(self.post_handshake_state(scf.into_next()));
                }
                InnerState::Handshake(ClientState::AwaitServerFlight(asf)) => {
                    if self.buffers.deframer_buffer.is_empty() {
                        self.state = Ok(InnerState::Handshake(ClientState::AwaitServerFlight(asf)));
                        break;
                    }
                    match asf.input_data(&mut self.buffers.deframer_buffer) {
                        Ok(state) => {
                            ...
                            if matches!(
                                self.state,
                                Ok(InnerState::Handshake(ClientState::AwaitServerFlight(_)))
                            ) {
                                break;
                            }
                        }
                ...
                InnerState::Handshake(st @ ClientState::Traffic(_)) => {
...
impl DerefMut for ClientConnection {
    fn deref_mut(&mut self) -> &mut Self::Target {
        match &mut self.state {
            Ok(InnerState::Handshake(hs)) => hs.deref_mut(),
            Ok(InnerState::Traffic { outputs, .. }) => outputs,
...
impl ClientConnectionBuilder {
    ...
    pub fn build(self) -> Result<ClientConnection, Error> {
        ...
        Ok(ClientConnection::new(
            ClientState::new(config, name, alpn_protocols)?,
            fips,
        ))
    }
}
