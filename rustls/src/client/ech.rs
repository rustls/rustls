...
impl EchMode {
    ...
    #[expect(dead_code)]
    pub(crate) fn fips(&self) -> FipsStatus {
        match self {
        ...
        for ech_config_list in ech_config_list_slice {
            match EchConfig::new(ech_config_list, suites) {
                ...
                Err(Error::InvalidEncryptedClientHello(
                    ...
                    // Continue past `NoCompatibleConfig` errors to try the next `ech_config_list`.
                }
    ...
    /// # Errors
    ///
    /// This method will error if the selected HPKE suite fails to generate a placeholder public key.
...
impl EchGreaseConfig {
    ...
    /// # Errors
    ///
    /// Returns an error if the provided HPKE suite fails to generate a placeholder public key.
    ///
    /// [^0]: <https://www.rfc-editor.org/rfc/rfc8701>
