...
impl EchGreaseConfig {
    ...
    pub(crate) fn grease_ext(
        ...
        outer_hello: &mut ClientHelloPayload,
...
impl EchState {
    ...
    pub(crate) fn ech_hello(
        ...
        let encoded_inner_hello = self.encode_inner_hello(&mut outer_hello, retry_req, resuming);
    ...
    fn encode_inner_hello(
        ...
        for ext in outer_extensions {
            ...
            if ext == ExtensionType::ServerName {
                ...
                if let Some(sni_value) = inner_sni {
                    inner_hello.extensions.server_name = Some(sni_value.into());
                }
            ...
            if ext.ech_compress() {
                ...
                outer_hello
                    .extensions
                    .copy_to(&mut inner_hello.extensions, ext);
            }
        }
        ...
        inner_hello
            .extensions
            .encrypted_client_hello_outer = Some(compressed_exts);
        ...
        let mut encoded_hello = inner_hello.ech_inner_encoding();
