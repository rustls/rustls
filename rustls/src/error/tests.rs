...
#[test]
fn error_with_alert() {
    let mut send_path = SendPath::new(Protocol::Tcp, Side::Server);
    send_path.set_negotiated_version(ProtocolVersion::TLSv1_2);
    let mut tls = Vec::new();
    let e = ErrorWithAlert::new(Error::NoApplicationProtocol, &mut send_path, &mut tls);
    assert_eq!(
        std::format!("{e:?}"),
