//! Implements parts of Datagram TLS (DTLS), specified in [RFC 6347][1] and [RFC 9147][2].
//!
...
use crate::conn::{ConnectionCommon, MessageIter};
...
use crate::{ClientConfig, ServerConfig, SideData, VecInput};
...
impl<SocketLike: UdpSocketLike> ClientDtlsSocket<SocketLike> {
    ...
    pub fn new(
        ...
        let connection_common = ConnectionCommon::for_client(
        ...
        Ok(Self {
            inner: DtlsSocket::new(inner, connection_common),
        })
    }
...
impl<SocketLike: UdpSocketLike> ServerDtlsSocket<SocketLike> {
    ...
    pub fn new(config: ServerConfig, inner: SocketLike) -> Result<Self, Error> {
        let connection_common = ConnectionCommon::for_server(
            Arc::new(config),
...
impl<SocketLike: UdpSocketLike, Side: SideData> DtlsSocket<SocketLike, Side> {
    fn new(inner: SocketLike, core: ConnectionCommon<Side>) -> Self {
        Self {
            inner,
            core,
            buffered_plaintext: ChunkVecBuffer::new(),
    ...
    fn send<B: AsRef<[u8]>>(&mut self, bytes: B) -> Result<usize, Error> {
        todo!("check if encryption is available and bufferplaintext if not");

        let mut buf = [0u8; 8096];
        self.pending_io(&mut buf)?;

        Ok(todo!("bytes sent or buffered"))
    }
    ...
    fn recv<B: AsMut<[u8]>>(&mut self, mut bytes: B) -> Result<usize, Error> {
        ...
        todo!("copy anything received into self.received_plaintext")
        // Ok(self
        //     .received_plaintext
        //     .append(&mut receive_buf)
        //     .map_err(|e| Error::Other(OtherError::new(e)))
    }
    ...
    fn pending_io(&mut self, mut read_into: &mut [u8]) -> Result<(), Error> {
        ...
        loop {
            ...
            let mut iter = MessageIter::new(
                &mut self.input,
                todo!("Vec to read into"),
                None,
                &mut self.core,
                true,
            );
            while let Some(result) = iter.next() {
                let payload = result?.reborrow(&Delocator::new(todo!("the tls buf used above"))); //iter.input().slice_mut()));
        ...
        {
            while let Some(buf) = self.buffered_plaintext.pop() {
                self.core
                    .send
                    .send_appdata_encrypt(buf.as_slice().into(), todo!("tls"));
            }
        }

        // Send any TLS messages we have to send out
        todo!("iterate over the tls buf passed to send_appdata_encrypt above");
        for sendable in ["todo"]
        //self.core.send.sendable_tls.take()
        {
            let _ = self
                .inner
                .send(sendable)
...
mod tests {
    ...
    use crate::crypto::{AntiReplay, Identity, SignatureScheme, TEST_PROVIDER, VerifiedIdentity};
    use crate::msgs::UnifiedHeader;
    ...
    fn full_handshake_and_application_data(version: AllowedTlsVersion) {
        let (mut client_socket, mut server_socket) = test_setup(version);
    ...
    fn anti_replay_dtls_12() {
        ...
        // Server recv: should reject replayed message.
        let err = server_socket
            .recv(&mut receive_buf)
            .unwrap_err();
    ...
    fn anti_replay_dtls13() {
        ...
        // Server recv: process client hello, send server hello, become ready for traffic
        assert_eq!(
            server_socket
                .recv(&mut receive_buf)
                .unwrap(),
        ...
        // Client recv: process server hello, become ready for traffic, send buffered message
        assert_eq!(
            client_socket
                .recv(&mut receive_buf)
                .unwrap(),
        ...
        let replayed = server_socket
            ...
            .unwrap()
            .clone();
        // Make sure the record has a unified header on it, indicating encrypted DTLS 1.3
        assert!(UnifiedHeader::is_unified_header(replayed[0]));
