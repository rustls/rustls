...
impl<'a, 'm, Side: SideData> MessageIter<'a, 'm, Side> {
    pub(crate) fn new(
        ...
        Self {
            locator: Locator::new(input.slice_mut()),
    ...
    pub(crate) fn next(&mut self) -> Option<Result<Option<UnborrowedPayload>, Error>> {
        ...
        let (message, want_close_before_decrypt) =
            if let Some(span) = self.recv.deframer.complete_span() {
                ...
                let (message, bounds) = match self.recv.deframe(buffer, &self.locator) {
                ...
                match unborrowed.typ {
                    ContentType::Handshake => {
                        ...
                        return match self.recv.deframer.coalesce(buffer) {
                            Ok(()) => {
                                *self.state = Ok(st);
                                Some(Ok(None))
                            }
...
impl<Side: SideData> Drop for MessageIter<'_, '_, Side> {
    fn drop(&mut self) {
...
struct JoinOutput<'a> {
    outputs: &'a mut ConnectionOutputs,
    quic: Option<&'a mut dyn QuicOutput>,
    send: &'a mut dyn SendOutput,
    side: &'a mut dyn SideOutput,
}
