...
use crate::{
    ...
    server::{ServerConfig, ServerSession, ServerSessionImpl},
    session::{Protocol, SessionCommon},
};
...
impl QuicExt for ClientSession {
    ...
    fn update_secrets(&self, client: &hkdf::Prk, server: &hkdf::Prk) -> Secrets {
        update_secrets(&self.imp.common, client, server)
    }
}
...
fn update_secrets(this: &SessionCommon, client: &hkdf::Prk, server: &hkdf::Prk) -> Secrets {
    ...
    let client = key_schedule::hkdf_expand(client, hkdf_alg, b"quic ku", &[]);
    let server = key_schedule::hkdf_expand(server, hkdf_alg, b"quic ku", &[]);
