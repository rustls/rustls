...
use crate::hkdf;
use crate::quic;
use crate::tls13::key_schedule::{hkdf_expand, hkdf_expand_aead_key};
...
impl HeaderProtectionKey {
    pub(crate) fn new(
        ...
        let key = hkdf_expand_aead_key(expander, alg.key_len(), version.header_key_label(), &[]);
...
impl PacketKey {
    pub(crate) fn new(
        ...
        let key = hkdf_expand_aead_key(
            expander,
            aead_algorithm.key_len(),
            version.packet_key_label(),
            &[],
        );
        let iv = hkdf_expand(expander, version.packet_iv_label(), &[]);
...
pub(crate) fn nonce_for(packet_number: u64, iv: &Iv) -> ring::aead::Nonce {
...
mod test {
    ...
    fn test_short_packet(version: Version, expected: &[u8]) {
        ...
        let expander =
            hkdf::Expander::from_okm(&hkdf::OkmOneBlock::from(SECRET), &ring::hmac::HMAC_SHA256);
    ...
    fn key_update_test_vector() {
        fn equal_okm(x: &hkdf::OkmOneBlock, y: &hkdf::OkmOneBlock) -> bool {
            x.as_ref() == y.as_ref()
        }

        let mut secrets = Secrets::new(
            // Constant dummy values for reproducibility
            hkdf::OkmOneBlock::from(
                &[
