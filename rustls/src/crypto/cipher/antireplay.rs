...
impl ReplayWindow {
    ...
    pub(crate) fn observe(&mut self, seq: FullRecordSequenceNumber) -> Result<(), AntiReplay> {
...
mod tests {
    ...
    fn reject_too_old() {
        ...
        window.observe(200.into()).unwrap();
        assert_eq!(window.next_expected_seq, 201);

        assert_eq!(window.observe(1.into()).unwrap_err(), AntiReplay::TooOld);
        // Boundary condition: the first sequence number that's outside the window
        assert_eq!(
            window
                .observe((200 - (128 + 1)).into())
                .unwrap_err(),
            AntiReplay::TooOld
        );
    }
    ...
    fn accept_newer() {
        let mut window = ReplayWindow::default();
        window.observe(1.into()).unwrap();
        assert_eq!(window.next_expected_seq, 2);
        window.observe(2.into()).unwrap();
        assert_eq!(window.next_expected_seq, 3);
        window.observe(400.into()).unwrap();
    ...
    fn accept_within_window() {
        ...
        window.observe(200.into()).unwrap();
        assert_eq!(window.next_expected_seq, 201);
        window.observe(150.into()).unwrap();
        assert_eq!(window.next_expected_seq, 201);
    }
    ...
    fn reject_within_window() {
        ...
        window
            .observe(left_edge.into())
            .unwrap();
        window.observe(middle.into()).unwrap();
        assert_eq!(
            window
                .observe(right_edge.into())
                .unwrap_err(),
            AntiReplay::Replay
        );
    ...
    fn slide_window() {
        ...
        // replay.
        window.observe(201.into()).unwrap();
        ...
        assert_eq!(window.observe(200.into()).unwrap_err(), AntiReplay::Replay);
    }
}
