...
///
/// A tag is "public" if it is published on the wire. For example, the
/// `verify_data` field of TLS `Finished` messages are public (as they
/// are published on the wire in TLS1.2, or sent encrypted under
/// pre-authenticated secrets in TLS1.3).
#[derive(Clone)]
