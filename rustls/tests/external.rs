...
fn err_on_handshake_not_complete() {
    ...
    assert!(matches!(
        client.dangerous_into_external_connection(),
        Err(Error::HandshakeNotComplete)
    ));
    assert!(matches!(
        server.dangerous_into_external_connection(),
        Err(Error::HandshakeNotComplete)
    ));
}
...
fn initial_traffic_secrets_match() {
    ...
    let (client_secrets, _) = client
        .dangerous_into_external_connection()
        .expect("failed to convert client connection to an ExternalConnection");
    let (server_secrets, _) = server
        .dangerous_into_external_connection()
        .expect("failed to convert server connection to an ExternalConnection");
