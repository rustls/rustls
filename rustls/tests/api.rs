...
fn client_only_attempts_resumption_with_compatible_security() {
    ...
    for version in rustls::ALL_VERSIONS {
        ...
        // allowed case, using `share_resumption_with`
        let mut client_config = make_client_config_with_versions(kt, &[version]);
        client_config.share_resumption_with(&base_client_config);
        let (mut client, mut server) =
            make_pair_for_configs(client_config.clone(), server_config.clone());
        do_handshake(&mut client, &mut server);
        assert_eq!(client.handshake_kind(), Some(HandshakeKind::Resumed));
        ...
        let old_creds = Arc::clone(&client_config.client_auth_cert_resolver);
        client_config.share_resumption_with(&base_client_config);
        client_config.client_auth_cert_resolver = old_creds;
        CountingLogger::reset();
        let (mut client, mut server) =
            make_pair_for_configs(client_config.clone(), server_config.clone());
        do_handshake(&mut client, &mut server);
