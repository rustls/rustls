...
use rustls::{
    server::ResolvesServerCert, sign::CertifiedKey, AlertDescription, CertificateError,
    DistinguishedName, Error, InvalidMessage, RootCertStore,
};
...
impl ResolvesServerCert for ResolvesCertChainByCaName {
    fn resolve(
        ...
        if let Some(cas_extension) = client_hello.certificate_authorities() {
            for (name, certified_key) in self.0.iter() {
                let name = x509_parser::x509::X509Name::from_der(name.as_ref())
                    .unwrap()
                    .1;
                if cas_extension.iter().any(|ca_name| {
                    x509_parser::x509::X509Name::from_der(ca_name.as_ref())
                        .is_ok_and(|(_, ca_name)| ca_name == name)
                }) {
                    println!("ResolvesCertChainByCaName: found matching CA name: {name}");
                    return Some(certified_key.clone());
                }
            }
            println!("ResolvesCertChainByCaName: no matching CA name found");
        }
        println!("ResolvesCertChainByCaName: returning default Cert");
...
impl ServerCertVerifier for ServerCertVerifierWithCasExt {
    ...
    fn root_hint_subjects(&self) -> Option<&[DistinguishedName]> {
        println!("ServerCertVerifierWithCertificateAuthorities::root_hint_subjects() called!");
        Some(&self.ca_names)
    }
}
...
fn client_can_request_certain_trusted_cas() {
            ...
            .map(|kt| {
                (
                    kt.ca_distinguished_name()
                        .to_vec()
                        .into(),
                    kt.get_certified_key_with_cert_chain()
                        .unwrap(),
                )
            })
            .collect(),
    ...
    for key_type in key_types {
        ...
        let ca_sending_client_config = client_config_builder()
            .dangerous()
            .with_custom_certificate_verifier(cas_sending_server_verifier)
            .with_no_client_auth();

        let (mut client, mut server) =
            make_pair_for_arc_configs(&Arc::new(ca_sending_client_config), &server_config);
        do_handshake(&mut client, &mut server);
